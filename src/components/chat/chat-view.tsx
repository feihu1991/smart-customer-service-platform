...
import { Send, Bot, User, Search, MoreVertical, Phone, ImagePlus, Smile, MessageCircle, ArrowLeft } from 'lucide-react'
...
export function ChatView() {
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-3rem)] md:h-[calc(100vh-3.5rem)] flex">
  ...
  const chatAreaContent = activeSession ? (
    <>
      {/* Chat Header */}
      <div className="h-14 border-b flex items-center justify-between px-3 md:px-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="w-8 h-8 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full flex items-center justify-center text-white text-sm font-bold md:hidden">
        ...
        <div className="space-y-4 max-w-3xl mx-auto">
          ...
          {activeSession.messages.map((msg) => (
            <div key={msg.id} className={`flex gap-2.5 ${msg.sender === 'customer_service' ? 'flex-row-reverse' : ''}`}>
              ...
              {/* Bubble */}
              <div className={`max-w-[75%] md:max-w-[70%] ${msg.sender === 'customer_service' ? 'text-right' : ''}`}>
      ...
      {/* Input Area */}
      <div className="border-t bg-white p-3 md:p-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] shrink-0">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-2 mb-2 md:mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-600 hidden sm:inline">AI自动回复</span>
  ...
  return (
    <div className="h-[calc(100vh-3rem)] md:h-[calc(100vh-3.5rem)] flex">
      ...
      <div className="hidden md:flex w-80 border-r bg-gray-50 flex-col shrink-0">
        {sessionListContent}
      </div>

      {/* Chat Area - desktop (always visible) */}
